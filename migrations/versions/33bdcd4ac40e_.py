...
def upgrade():
    ...
    op.create_table('test_type',
        ...
        sa.Column('group_id', Integer),
        sa.Column('is_group', Boolean),
    )
...
def downgrade():
    ...
    op.execute(sql='DROP TABLE test_type CASCADE;')
    ...
    # we drop tables and on start app will create them from models
    op.execute(sql='DROP TABLE campaign CASCADE;')
    op.execute(sql='DROP TABLE test_result CASCADE;')
    op.execute(sql='DROP TABLE test_result_param_value CASCADE;')
